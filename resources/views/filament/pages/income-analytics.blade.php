    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm no-print">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-lg font-bold text-gray-900 dark:text-white">Filter Report Range</h2>
                    <p class="text-xs text-gray-500 dark:text-gray-400">Select month and year for detailed insights</p>
                </div>
            </div>
        ...
        <div class="hidden print:block border-b border-gray-300 pb-4 mb-6">
            <div class="flex justify-between items-end">
                <div>
                    <h1 class="text-2xl font-bold text-black">TechQueen Business Performance Report</h1>
                    <p class="text-sm text-gray-600">Generated on: {{ now()->format('Y-m-d') }}</p>
                </div>
                <div class="text-right">
                    <h2 class="text-lg font-bold text-gray-800">{{ $this->months[$this->month] }} {{ $this->year }}</h2>
                    <p class="text-xs text-gray-600">Total Models Created: {{ $this->stats['total_jobs'] }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="analytics-grid stats-row grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 print-full-width">
            ...
            <div class="print-card bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6 shadow-sm relative overflow-hidden flex flex-col justify-between">
                <div class="flex items-center justify-between mb-4">
                    <span class="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Estimated Revenue</span>
                ...
                <div>
                    <h3 class="text-3xl font-extrabold text-gray-900 dark:text-white">
                        ...
                        @if (!$hidePrices) <span class="text-sm font-semibold text-gray-500">EGP</span> @endif
                    </h3>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Based on price of finished/paid jobs</p>
                </div>
            ...
            <div class="print-card bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6 shadow-sm relative overflow-hidden flex flex-col justify-between">
                <div class="flex items-center justify-between mb-4">
                    <span class="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Collected Income</span>
            ...
            <div class="print-card bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6 shadow-sm relative overflow-hidden flex flex-col justify-between">
                <div class="flex items-center justify-between mb-4">
                    <span class="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Unpaid Receivables</span>
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Pending payments from active models</p>
                </div>
            ...
            <div class="print-card bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6 shadow-sm relative overflow-hidden flex flex-col justify-between">
                <div class="flex items-center justify-between mb-4">
                    <span class="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Jobs & Model Completion</span>
                ...
                <div>
                    <h3 class="text-3xl font-extrabold text-gray-900 dark:text-white">{{ $this->stats['total_jobs'] }} <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Models</span></h3>
                    <div class="flex flex-wrap gap-1 mt-2">
                        <span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400">Done: {{ $this->stats['completed_count'] }}</span>
                        <span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400">Active: {{ $this->stats['in_progress_count'] }}</span>
                        <span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-400">Delayed: {{ $this->stats['delayed_count'] }}</span>
                        <span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300">Hold: {{ $this->stats['on_hold_count'] }}</span>
                    </div>
                </div>
        ...
        <div class="analytics-grid tables-row grid grid-cols-1 lg:grid-cols-3 gap-6 print-full-width">
            ...
            <div class="print-card bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6 shadow-sm overflow-hidden flex flex-col justify-between lg:col-span-2">
                <div class="mb-4 border-b border-gray-100 dark:border-gray-800 pb-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        <h3 class="text-base font-bold text-gray-900 dark:text-white">Monthly Models List</h3>
                        <p class="text-xs text-gray-500 dark:text-gray-400">All models scheduled or delivered in {{ $this->months[$this->month] }}</p>
                    </div>
                    <span class="text-xs font-bold text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-500/10 px-2.5 py-1 rounded-full border border-primary-100 dark:border-primary-500/20 self-start sm:self-center">
                        Total Value: {{ $hidePrices ? '***' : number_format($this->stats['total_revenue'], 0) . ' EGP' }}
                    </span>
                </div>
                    ...
                    <table class="w-full text-left text-xs border-collapse">
                        <thead>
                            <tr class="border-b border-gray-100 dark:border-gray-800 pb-2 text-gray-500 dark:text-gray-400 uppercase tracking-wider font-semibold">
                                <th class="pb-2">Model Piece</th>
                                <th class="pb-2">Client</th>
                                <th class="pb-2 text-center">Dates</th>
                                <th class="pb-2 text-right">Price</th>
                                <th class="pb-2 text-right">Deposit</th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800/40 text-gray-700 dark:text-gray-300">
                            @forelse ($this->monthlyModels as $model)
                                <tr>
                                    ...
                                    <td class="py-3">{{ $model->client?->name ?? 'Walk-in Client' }}</td>
                                    <td class="py-3 text-center">
                                        <span class="block text-[10px]">Rec: {{ $model->receiving_date }}</span>
                                        <span class="block text-[10px] font-bold text-gray-500 dark:text-gray-400">Del: {{ $model->delivery_date }}</span>
                                    </td>
                                    <td class="py-3 text-right font-semibold">
                                        {{ $hidePrices ? '***' : number_format($model->price, 0) . ' EGP' }}
                                    </td>
                                    <td class="py-3 text-right text-gray-500 dark:text-gray-400">
                                        {{ $hidePrices ? '***' : number_format($model->deposit, 0) . ' EGP' }}
                                    </td>
                                    <td class="py-3 text-right font-bold {{ $model->price - $model->deposit > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-gray-400' }}">
                                        {{ $hidePrices ? '***' : number_format(max(0, $model->price - $model->deposit), 0) . ' EGP' }}
                                    </td>
                                    <td class="py-3 text-center">
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="py-8 text-center text-gray-400 dark:text-gray-500">
                                        No models registered for this month
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
            ...
            <div class="print-card bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6 shadow-sm overflow-hidden flex flex-col justify-between">
                <div class="mb-4 border-b border-gray-100 dark:border-gray-800 pb-3">
                    <h3 class="text-base font-bold text-gray-900 dark:text-white">Top Clients by Revenue</h3>
                    <p class="text-xs text-gray-500 dark:text-gray-400">Leading contributors for this month</p>
                </div>
                    ...
                    <table class="w-full text-left text-xs border-collapse">
                        <thead>
                            <tr class="border-b border-gray-100 dark:border-gray-800/80 pb-2 text-gray-500 dark:text-gray-400 uppercase tracking-wider font-semibold">
                                <th class="pb-2">Client</th>
                                <th class="pb-2 text-center">Jobs</th>
                            ...
                            @forelse ($this->topClients as $client)
                                <tr>
                                    ...
                                    <td class="py-2.5 text-right font-extrabold text-emerald-600 dark:text-emerald-400">
                                        {{ $hidePrices ? '***' : number_format($client['revenue'], 0) . ' EGP' }}
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="py-6 text-center text-gray-400 dark:text-gray-500">No client data found for this month</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
